...
public class FeatureInstaller {
	...
	public byte[] addFeaturesToInstall(InputStream inputStream, boolean isPom) throws TransformerException, IOException, ParserConfigurationException, SAXException {
		Document doc = DomUtil.getDocument(inputStream);
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		writeToStream(doc, baos, isPom);
		return baos.toByteArray();
	}

	private void writeToStream(Document doc, OutputStream outputStream, boolean isPom) throws TransformerFactoryConfigurationError, TransformerException {
		if(!listOfFeatures.isEmpty()){
			...
			if(isPom){
				...
				features = DomUtil.findOrAddChildNode(doc, server, "featureManager", null);
			}
	...
	private void enforceAcceptLicenseProperty(Document doc, Node pluginsNode){
		...
		if(propertyNode == null){
			...
			DomUtil.addChildNode(doc, requireProperty, "message", "You must set a value for the 'accept.features.license' property defined in myProject-wlpcfg/pom.xml. Please review the license terms and conditions for additional features to be installed and if you accept the license terms and conditions then run the Maven command with '-Daccept.features.license=true'.");
			DomUtil.addChildNode(doc, requireProperty, "regex", "true");
			DomUtil.addChildNode(doc, requireProperty, "regexMessage", "Additional features could not be installed as the license terms and conditions were not accepted. If you accept the license terms and conditions then run the Maven command with '-Daccept.features.license=true'.");
		}
